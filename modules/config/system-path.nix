  ...
  requiredPackages =
    map (pkg: lib.setPrio ((pkg.meta.priority or lib.meta.defaultPriority) + 3) pkg)
      [
        ...
        pkgs.ncurses
        pkgs.netcat
        # FIXME: need custom stripped-down ssh module without systemd
...

{
  options = {
    ...
    build = {
      # This is consumed from nix2container.nix, required to determine which packages
      # form the base layer of the OS.
